...
// This code uses ECB mode and should not be used in any serious cryptographic context.
class Blowfish
{
  public:
  ...
  void set_key(const char str[])
  {
    ...
    if (key_length > 56)
    {
      printf("Warning: Key is more than 56 bytes long. Using the first 56.\n");
      key_length = 56;
    }
    ...
    if (key_length < 4)
    {
      printf("Warning: Key is less than 4 bytes long. Padding with 0.\n");
      key_length = 4;
    }
  ...
  void set_pad_byte(const uint8_t b)
  {
    pad_byte = b;
    return;
  }
  ...
  // Appends '0x0A' (line feed) in the case the file size is
  // not multiple of 8 bytes. Can be changed using 'void set_pad_byte(uint8_t)'
  bool encrypt_file(const char input_file_name[], const char output_file_name[])
  {
    ...
    else
    {
      ...
      int input_EOF = ftell(input);
      fseek(input, 0, SEEK_SET);

      uint32_t buffer[2];

      // Encrypt all but the last 64-bit chunk. We don't know if it's 
      // gonna be exactly 64 bits.
      while (ftell(input) != (input_EOF - input_EOF % 8))
      {
         // Endian-dependent !!
        fread(buffer, sizeof(uint32_t), 2, input);
        encrypt(buffer, buffer + 1);
        fwrite(buffer, sizeof(uint32_t), 2, output);
      }

      buffer[0] = 0; // left
      buffer[1] = 0; // right

      // If file size is not multiple of 8 bytes, encode byte by byte appending with "pad_byte".
      if (input_EOF % 8 != 0)
      {
        for (int i = 0; i < 8; i++)
        {
          if (i < input_EOF % 8)
          {
            uint8_t aux;
            fread(&aux, 1, 1, input);
            buffer[i / 4] |= (uint32_t) aux << i * 8;
          }
          else
            buffer[i / 4] |= (uint32_t) pad_byte << i * 8;
        }
  ...
  // Assumes file size multiple of 8 bytes. After all it was encoded
  // with blowfish, 64-bit block size...
  bool decrypt_file(const char input_file_name[], const char output_file_name[])
  {
    FILE* input = fopen(input_file_name, "rb");
    FILE* output = fopen(output_file_name, "wb");
    if (input == NULL)
    ...
    else
    {
      ...
      while (ftell(input) != input_EOF)
      {
        fread(buffer, sizeof(uint32_t), 2, input);
        decrypt(buffer, buffer + 1);
        fwrite(buffer, sizeof(uint32_t), 2, output);
      }
  ...
  private:
  // Globals
  uint8_t pad_byte = 0x0A; // Default to padding with line feed so that decrypted ascii text can be 
                          // recognized by text editors
  uint8_t key[56] = {0};
  uint32_t key_length = 0; // Length in bytes
  uint32_t P[18] = { 
